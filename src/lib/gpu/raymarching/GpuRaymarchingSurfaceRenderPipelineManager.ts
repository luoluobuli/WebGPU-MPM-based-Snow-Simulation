...
import type { GpuVolumetricBufferManager } from "../volumetric/GpuVolumetricBufferManager";
...
import calculateGridMassSrc from "../volumetric/calculateGridMass.wgsl?raw";
import surfaceRaymarchSrc from "./surfaceRaymarch.cs.wgsl?raw";
import volumetricVertexSrc from "../volumetric/volumetricVertex.wgsl?raw";
import volumetricFragmentSrc from "../volumetric/volumetricFragment.wgsl?raw";
import preludeSrc from "../volumetric/prelude.wgsl?raw";
...
const prerenderPasses = [
    "surface raymarch compute",
];

export class GpuRaymarchingSurfaceRenderPipelineManager implements GpuRenderMethod {
    readonly massCalulationPipeline: GPUComputePipeline;
    readonly raymarchPipeline: GPUComputePipeline;
    ...
    constructor({
        ...
        const massCalulationBindGroupLayout = device.createBindGroupLayout({
            label: "surface raymarch mass calculation bind group layout",
            entries: [
                {
                    binding: 0,
        ...
        this.massCalulationBindGroup = device.createBindGroup({
            label: "surface raymarch mass calculation bind group",
        ...
        const massCalulationPipelineLayout = device.createPipelineLayout({
            label: "surface raymarch mass calculation pipeline layout",
        ...
        this.massCalulationPipeline = device.createComputePipeline({
            label: "surface raymarch mass calculation pipeline",
        ...
        const raymarchBindGroupLayout = device.createBindGroupLayout({
            label: "surface raymarch bind group layout",
            entries: [
                {
                    binding: 0,
                    visibility: GPUShaderStage.COMPUTE,
        ...
        this.raymarchBindGroup = device.createBindGroup({
            label: "surface raymarch bind group",
        ...
        const raymarchPipelineLayout = device.createPipelineLayout({
            label: "surface raymarch pipeline layout",
            bindGroupLayouts: [
        ...
        this.raymarchPipeline = device.createComputePipeline({
            label: "surface raymarch pipeline",
            layout: raymarchPipelineLayout,
            compute: {
                module: device.createShaderModule({
                    code: attachPrelude(`${preludeSrc}\n${surfaceRaymarchSrc}`),
                }),
                entryPoint: "doSurfaceRaymarch",
            },
        });

        const renderBindGroupLayout = device.createBindGroupLayout({
            label: "surface render bind group layout",
            entries: [
                {
                    binding: 0,
        ...
        this.renderBindGroup = device.createBindGroup({
            label: "surface render bind group",
        ...
        const renderPipelineLayout = device.createPipelineLayout({
            label: "surface render pipeline layout",
            bindGroupLayouts: [renderBindGroupLayout],
        });

        this.renderPipeline = device.createRenderPipeline({
            label: "surface render pipeline",
    ...
    addRaymarchDispatch(computePassEncoder: GPUComputePassEncoder) {
        computePassEncoder.setPipeline(this.raymarchPipeline);
        computePassEncoder.setBindGroup(0, this.uniformsManager.bindGroup);
    ...
    addPrerenderPasses(commandEncoder: GPUCommandEncoder, depthTextureView: GPUTextureView) {
        ...
        const prerenderComputePassEncoder = commandEncoder.beginComputePass({
            label: "surface raymarch compute pass",

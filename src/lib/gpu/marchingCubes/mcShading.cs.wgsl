...
const AMBIENT_COLOR = vec3f(0.186, 0.19, 0.2);
...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    let diffuseNormal = normalize(mix(normal, perturbedNormal, 0.3));
    ...
    let diffuse = vec3f(0.92, 0.94, 0.98) * wrapDiffuse;
    ...
    let baseSpecular = (D * G * F) / max(4.0 * NdotV * NdotL, 0.001) * NdotL * 0.3;
    ...
    let sss = subsurfaceScattering(normal, V, L, thickness);
    
    // ===== STEP 5: Light glints with PERTURBED normal =====
    let glint = glintMask(worldPos, specularNormal, L, V);
    let glintSpec = coatSpecular(specularNormal, V, L, glint);
    
    // ===== Fresnel rim lighting =====
    let fresnel = pow(1 - max(dot(diffuseNormal, V), 0), 3);
    let rim = fresnel * 0.05 * vec3f(0.9, 0.95, 1);
